# FantasyRolePlaying Python SDK

The Python SDK for the FantasyRolePlaying API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Entity(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.FantasyRolePlaying(None).load(
    {"id": "test01"}, None
)
