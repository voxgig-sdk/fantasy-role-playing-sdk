...
class TestRollEntity:
    ...
    def test_should_stream(self):
        ...
        base = FantasyRolePlayingSDK.test(seed, None)
        seen = list(base.Roll(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from fantasyroleplaying_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = FantasyRolePlayingSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Roll(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _roll_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "roll." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        roll_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.roll")))
        roll_ref01_data = None
        if len(roll_ref01_data_raw) > 0:
...
def _roll_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/roll/RollTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FANTASY_ROLE_PLAYING_TEST_LIVE") == "TRUE":
        ...
        client = FantasyRolePlayingSDK(helpers.to_map(merged_opts))

    _live = env.get("FANTASY_ROLE_PLAYING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FANTASY_ROLE_PLAYING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
