# FantasyRolePlaying SDK test runner
...
class FantasyRolePlayingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FantasyRolePlayingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FantasyRolePlayingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FantasyRolePlayingTestRunner.getenv("FANTASY_ROLE_PLAYING_TEST_LIVE")
        override = FantasyRolePlayingTestRunner.getenv("FANTASY_ROLE_PLAYING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FantasyRolePlayingTestRunner.getenv(key)
        ...
        explain = FantasyRolePlayingTestRunner.getenv("FANTASY_ROLE_PLAYING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FantasyRolePlayingTestRunner._test_control is not None:
            return FantasyRolePlayingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FantasyRolePlayingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FantasyRolePlayingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FantasyRolePlayingTestRunner.load_test_control()
...
def load_env_local():
    FantasyRolePlayingTestRunner.load_env_local()


def env_override(m):
    return FantasyRolePlayingTestRunner.env_override(m)


def entity_data(v):
    return FantasyRolePlayingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FantasyRolePlayingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FantasyRolePlayingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FantasyRolePlayingTestRunner.load_test_control()


def live_client_options():
    return FantasyRolePlayingTestRunner.live_client_options()


def live_delay_ms():
    return FantasyRolePlayingTestRunner.live_delay_ms()
