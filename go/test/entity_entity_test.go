...
func TestEntityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		entityRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.entity")))
...
func entityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID"])
