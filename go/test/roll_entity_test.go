...
func TestRollEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Roll(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Roll(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		rollRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.roll")))
...
func rollBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID"])
