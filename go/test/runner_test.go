...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FANTASYROLEPLAYING_TEST_LIVE") == "TRUE" ||
		os.Getenv("FANTASYROLEPLAYING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FANTASYROLEPLAYING_TEST_EXPLAIN"); explain != "" {
