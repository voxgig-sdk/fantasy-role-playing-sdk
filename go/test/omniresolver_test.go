...
import (
	...
	omni "github.com/voxgig-sdk/fantasy-role-playing-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FantasyRolePlayingError); is {
				out := map[string]any{
					"name":    "FantasyRolePlayingError",
...
func omniCtx(arg any, client *sdk.FantasyRolePlayingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FantasyRolePlayingSDK); is {
			client = live
