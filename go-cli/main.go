// Command FantasyRolePlaying-cli is a CLI + REPL for the FantasyRolePlaying SDK,
...
const prompt = "fantasy-role-playing"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "entity roll"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FANTASY_ROLE_PLAYING_APIKEY (api key), FANTASY_ROLE_PLAYING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FANTASY_ROLE_PLAYING_APIKEY carries the API key and
	// FANTASY_ROLE_PLAYING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FANTASY_ROLE_PLAYING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FANTASY_ROLE_PLAYING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFantasyRolePlayingSDK(opts)
