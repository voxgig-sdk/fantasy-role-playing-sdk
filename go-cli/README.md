...
boru-driven command-line client **and** interactive REPL for the FantasyRolePlaying
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fantasy-role-playing-cli)
make build

# 2. See usage (words, entities, env vars)
./fantasy-role-playing-cli --help
...
./fantasy-role-playing-cli list roll

# 5. Override the API base URL for a single call
FANTASY_ROLE_PLAYING_BASE=https://api.example.com ./fantasy-role-playing-cli list entity

# 6. No arguments -> interactive REPL
./fantasy-role-playing-cli
fantasy-role-playing> list entity
fantasy-role-playing> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fantasy-role-playing-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fantasy-role-playing-cli list entity
   ```
...
Run with no arguments to open a REPL (prompt `fantasy-role-playing>`). Each line is
evaluated as its own boru expression:

```text
$ ./fantasy-role-playing-cli
fantasy-role-playing> list entity
fantasy-role-playing> /help
fantasy-role-playing> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fantasy-role-playing-cli`. |
...
(`./fantasy-role-playing-cli <expr>`) and interactively (the REPL), and expressions compose the
