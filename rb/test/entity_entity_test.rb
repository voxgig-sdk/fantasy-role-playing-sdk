...
class EntityEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Entity(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Entity(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def entity_basic_setup(extra)
  ...
  entid_env_raw = ENV["FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID"])
