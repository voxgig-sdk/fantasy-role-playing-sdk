...
class RollEntityTest < Minitest::Test
  def test_create_instance
    testsdk = FantasyRolePlayingSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = FantasyRolePlayingSDK.test(seed, nil)
    seen = base.Roll(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = FantasyRolePlayingConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = FantasyRolePlayingSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Roll(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = roll_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "roll." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    roll_ref01_data = nil
    if roll_ref01_data_raw.length > 0
...
def roll_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "roll", "RollTestData.json")
