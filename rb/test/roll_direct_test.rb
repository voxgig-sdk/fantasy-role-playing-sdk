...
class RollDirectTest < Minitest::Test
  def test_direct_list_roll
    setup = roll_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-roll", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_roll
    setup = roll_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-roll", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def roll_direct_setup(mockres)
  ...
  if live
    ...
    client = FantasyRolePlayingSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
