...
class RollEntity
  ...
  def make
    opts = @_entopts.dup
    RollEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FantasyRolePlayingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Roll, Hash] the current Roll data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FantasyRolePlayingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Roll fields)
  def match_get
  ...
  # Load a single Roll.
  #
  # @param reqmatch [RollLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Roll.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Roll, Hash] the loaded Roll; raises FantasyRolePlayingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FantasyRolePlayingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [RollListMatch, Hash, nil] match filter (any subset of
  #   Roll fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Roll>, Array] the matching Roll items; raises FantasyRolePlayingError on failure
