...
require_relative 'FantasyRolePlaying_types'
...
class FantasyRolePlayingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FantasyRolePlayingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FantasyRolePlayingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FantasyRolePlayingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FantasyRolePlayingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FantasyRolePlayingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FantasyRolePlayingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FantasyRolePlayingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FantasyRolePlayingError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FantasyRolePlayingError => err
    ...
    if fetched.is_a?(Hash)
      status = FantasyRolePlayingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FantasyRolePlayingError.new(
        "graphql_error", "FantasyRolePlayingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FantasyRolePlayingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
