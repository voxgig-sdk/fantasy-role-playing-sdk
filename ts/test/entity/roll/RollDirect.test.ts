...
describe('RollDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new FantasyRolePlayingSDK({
  ...
  test('direct-load-roll', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-roll', setup.live)) return
  ...
  test('direct-list-roll', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-roll', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID']
