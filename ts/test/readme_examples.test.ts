...
//     (\`new FantasyRolePlayingSDK(...)\` / \`FantasyRolePlayingSDK.test(...)\` become
//     \`FantasyRolePlayingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FantasyRolePlayingSDK'
...
const TEST_SEED = {"entity":{"entity":{"test01":{"id":"test01"}},"roll":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FantasyRolePlayingSDK(...)\` and any
// \`FantasyRolePlayingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FantasyRolePlayingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FantasyRolePlayingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
