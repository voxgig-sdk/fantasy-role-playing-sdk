...
(0, node_test_1.describe)('RollDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FantasyRolePlayingSDK({
    ...
    (0, node_test_1.test)('direct-load-roll', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-roll', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-roll', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-roll', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.FANTASY_ROLE_PLAYING_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FantasyRolePlayingSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FANTASY_ROLE_PLAYING_TEST_ROLL_ENTID'];
    ...
    const client = new __1.FantasyRolePlayingSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=RollDirect.test.js.map