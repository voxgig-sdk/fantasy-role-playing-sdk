...
local function make_error_util(ctx, err)
  ...
  local msg = "FantasyRolePlayingSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = FantasyRolePlayingError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == FantasyRolePlayingError then
