...
The Lua SDK for the FantasyRolePlaying API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Entity(nil):list(nil, nil)
...
local result, err = client:FantasyRolePlaying(nil):load(
  { id = "test01" }, nil
)
