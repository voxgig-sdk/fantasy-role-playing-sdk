# FantasyRolePlaying Lua SDK



The Lua SDK for the FantasyRolePlaying API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Entity()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/fantasy-role-playing-sdk/releases)),
...
The main module (`fantasy-role-playing_sdk`) exports the SDK constructor
