...
describe("EntityEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local entity_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.entity")))
    local entity_ref01_data = nil
    if #entity_ref01_data_raw > 0 then
...
function entity_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FANTASY_ROLE_PLAYING_TEST_ENTITY_ENTID")
