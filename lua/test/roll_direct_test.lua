...
describe("RollDirect", function()
  it("should direct-list-roll", function()
    local setup = roll_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-roll", setup.live and "live" or "unit")
  ...
  it("should direct-load-roll", function()
    local setup = roll_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-roll", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
