...
local RollEntity = {}
RollEntity.__index = RollEntity


function RollEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RollEntity)
  self._name = "roll"
  self._client = client
...
function RollEntity:get_name()
  return self._name
end


function RollEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RollEntity.new(self._client, opts)
end
...
function RollEntity:mark_deleted()
  self._deleted = true
end


function RollEntity:deleted()
  return true == self._deleted
end


function RollEntity:data_set(args)
  if args ~= nil then
...
function RollEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RollEntity:match_set(args)
  if args ~= nil then
...
function RollEntity:stream(action, args, callopts)
...
function RollEntity:_run_op(ctx, post_done)
  local utility = self._utility
