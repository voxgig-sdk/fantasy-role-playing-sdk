# FantasyRolePlaying Lua SDK Reference

Complete API reference for the FantasyRolePlaying Lua SDK.
...
```lua
local results, err = client:Roll(nil):list(nil, nil)
```
...
```lua
local result, err = client:Roll(nil):load({ id = "roll_id" }, nil)
```
