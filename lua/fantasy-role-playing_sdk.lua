...
require("fantasy-role-playing_types")
...
local FantasyRolePlayingSDK = {}
FantasyRolePlayingSDK.__index = FantasyRolePlayingSDK
...
FantasyRolePlayingSDK._make_feature = _make_feature


function FantasyRolePlayingSDK.new(options)
  local self = setmetatable({}, FantasyRolePlayingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FantasyRolePlayingSDK:options_map()
...
function FantasyRolePlayingSDK:get_utility()
  return Utility.copy(self._utility)
end


function FantasyRolePlayingSDK:get_root_ctx()
  return self._rootctx
end


function FantasyRolePlayingSDK:prepare(fetchargs)
...
function FantasyRolePlayingSDK:direct(fetchargs)
...
function FantasyRolePlayingSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function FantasyRolePlayingSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "FantasyRolePlayingSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function FantasyRolePlayingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FantasyRolePlayingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FantasyRolePlayingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FantasyRolePlayingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FantasyRolePlayingSDK.new(sdkopts)
  sdk.mode = "test"
