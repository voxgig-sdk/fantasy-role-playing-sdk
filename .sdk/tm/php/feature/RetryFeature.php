...
// FantasyRolePlaying SDK retry feature
...
class FantasyRolePlayingRetryFeature extends FantasyRolePlayingBaseFeature
{
    ...
    public function init(FantasyRolePlayingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FantasyRolePlayingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FantasyRolePlayingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FantasyRolePlayingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
