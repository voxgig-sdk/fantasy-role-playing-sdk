...
// FantasyRolePlaying SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FantasyRolePlayingTestFeature extends FantasyRolePlayingBaseFeature
{
    ...
    public function init(FantasyRolePlayingContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FantasyRolePlayingContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FantasyRolePlayingParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FantasyRolePlayingContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FantasyRolePlayingContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FantasyRolePlayingParam::call($ctx, $k);
