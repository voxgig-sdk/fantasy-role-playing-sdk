...
//    only - a typed FantasyRolePlayingContext there would make every ctx
...
final class FantasyRolePlayingOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FantasyRolePlayingContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FantasyRolePlayingSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FantasyRolePlayingResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FantasyRolePlayingError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FantasyRolePlayingResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FantasyRolePlayingError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FantasyRolePlayingError(is_string($code) ? $code : '', $msg);
    }
