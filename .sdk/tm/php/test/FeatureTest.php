...
// FantasyRolePlaying SDK feature test
//
...
class FtCtrl extends FantasyRolePlayingControl
{
...
class FtClient
{
    ...
    private FantasyRolePlayingUtility $utility;

    public function __construct(FantasyRolePlayingUtility $utility, array $options)
    {
    ...
    public function get_utility(): FantasyRolePlayingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FantasyRolePlayingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FantasyRolePlayingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FantasyRolePlayingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FantasyRolePlayingConfig::make_feature($name);
    ...
    private static function buildUrl(FantasyRolePlayingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FantasyRolePlayingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FantasyRolePlayingResponse($fetched);
        }
    ...
    private function fail(FantasyRolePlayingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FantasyRolePlayingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
