...
// FantasyRolePlaying SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FantasyRolePlayingOmni::makeStructRunner(
    ...
    {
        return FantasyRolePlayingOmni::objmodel(FantasyRolePlayingOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return FantasyRolePlayingOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return FantasyRolePlayingOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FantasyRolePlayingOmni::structfix($now, Struct::undef()))
                        !== json_encode(FantasyRolePlayingOmni::structfix($val, Struct::undef()))) {
