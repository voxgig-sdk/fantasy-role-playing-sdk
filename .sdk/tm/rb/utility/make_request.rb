...
module FantasyRolePlayingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FantasyRolePlayingResponse.new({})
    ...
    if fetch_err
      ...
      response = FantasyRolePlayingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FantasyRolePlayingResponse.new(fetched)
    else
